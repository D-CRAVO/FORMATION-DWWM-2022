...
int[] tableau_occurences = new int[26];
...
for (compteur_alphabet = 0; compteur_alphabet < tableau_alphabet.Length; compteur_alphabet++)
{
    ...
    tableau_occurences[compteur_alphabet] = compteur_occurence;
}

// On affiche le résultat de la recherche.
for (compteur_alphabet = 0; compteur_alphabet < tableau_alphabet.Length; compteur_alphabet++)
{
    if (tableau_occurences[compteur_alphabet] != 0)
    {
        Console.WriteLine("La lettre " + tableau_alphabet[compteur_alphabet] + " apparaît " + tableau_occurences[compteur_alphabet] + " fois dans le texte.");
    }
}
